    ...
    for (match=0; match<L2_ST_WAY; match++) {
        if (table[match].valid && (table[match].tag == tag)) { // Hit 
            ...
            if (table[match].signature == 0) { // First hit in L2_ST
                ...
                if (sig_delta < 0)
                    sig_delta = 32768 + (curr_block - table[match].last_block)*(-1);
    ...
    for (match=0; match<L2_ST_WAY; match++) {
        if (table[match].valid && (table[match].tag == tag)) {
            ...
            L2_PF_DEBUG(printf("ST_check found cpu: %d cl_addr: %lx page: %lx block: %ld old_sig: %x last_block: %d\n", 
                        cpu, addr >> LOG2_BLOCK_SIZE, curr_page, (addr >> LOG2_BLOCK_SIZE) & 0x7FFF, table[match].signature, table[match].last_block));
            return match;
        }
    }

    if (warmup_complete[cpu])
    L2_PF_DEBUG(printf("ST_check not found cpu: %d cl_addr: %lx page: %lx block: %ld\n", cpu, addr >> LOG2_BLOCK_SIZE, curr_page, (addr >> LOG2_BLOCK_SIZE) & 0x7FFF));
    return -1;
}
